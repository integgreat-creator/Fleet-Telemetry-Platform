...
function _cors(req: Request): Record<string, string> {
  const o = req.headers.get('Origin') ?? '';
  const allow = _ALLOWED.includes('*') || _ALLOWED.includes(o) ? (o || '*') : '';
  return {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'detect-trips') {
      return await handleTripDetection(supabase, corsHeaders);
    }
    if (action === 'populate-behavior') {
      return await handlePopulateBehavior(supabase, corsHeaders);
    }
    if (action === 'calculate-scores') {
      return await handleDriverScoring(supabase, corsHeaders);
    }
    if (action === 'generate-insights') {
      return await handleInsightGeneration(supabase, corsHeaders);
    }
    if (action === 'predict-fuel') {
      return await handleFuelPrediction(supabase, corsHeaders);
    }
    if (action === 'optimize-route') {
      return await handleRouteOptimization(supabase, url, corsHeaders);
    }
    if (action === 'predict-costs') {
      return await handleCostPrediction(supabase, corsHeaders);
    }
    if (action === 'detect-anomalies') {
      return json(await handleAnomalyDetection(supabase), corsHeaders);
    }
    if (action === 'detect-gaps') {
      return json(await handleGapDetection(supabase), corsHeaders);
    }
    if (action === 'score-trip-data') {
      return json(await handleTripScoring(supabase), corsHeaders);
    }
...
// ── Derive driver behavior events from raw sensor readings ───────────────────
async function handlePopulateBehavior(supabase: any, corsHeaders: Record<string, string>) {
  const { data: vehicles } = await supabase.from('vehicles').select('id');
  if (!vehicles) return ok('No vehicles found', corsHeaders);
  ...
  return ok('Driver behavior populated from sensor data', corsHeaders);
}

async function handleTripDetection(supabase: any, corsHeaders: Record<string, string>) {
  const { data: vehicles } = await supabase.from('vehicles').select('id');
  if (!vehicles) return ok('No vehicles', corsHeaders);
  ...
  for (const vehicle of vehicles) {
    ...
    if (!activeTrip && currentSpeed > 5) {
      ...
      const { data: speedData } = await supabase
        .from('sensor_data').select('value')
        .eq('vehicle_id', vehicle.id).eq('sensor_type', 'speed')
        .gte('timestamp', activeTrip.start_time).limit(1000);

      const { data: fuelData } = await supabase
        .from('sensor_data').select('value')
        .eq('vehicle_id', vehicle.id).eq('sensor_type', 'engineFuelRate')
        .gte('timestamp', activeTrip.start_time).limit(1000);
  ...
  return ok('Trip detection processed', corsHeaders);
}

async function handleDriverScoring(supabase: any, corsHeaders: Record<string, string>) {
  const { data: behaviorData } = await supabase.from('driver_behavior').select('*');
  if (!behaviorData) return ok('No behavior data', corsHeaders);
  ...
  for (const behavior of behaviorData) {
    const score = Math.max(0, Math.min(100,
      ...
      - ((behavior.average_engine_load ?? 0) * 0.1)
    ));
    await supabase.from('driver_behavior')
      .update({ driver_score: Math.round(score) })
      .eq('id', behavior.id);
  }

  return ok('Scores recalculated', corsHeaders);
}

async function handleFuelPrediction(supabase: any, corsHeaders: Record<string, string>) {
  const { data: vehicles } = await supabase.from('vehicles').select('*');
  if (!vehicles) return ok('No vehicles', corsHeaders);

  for (const vehicle of vehicles) {
    const { data: behavior } = await supabase
      .from('driver_behavior').select('*')
      .eq('vehicle_id', vehicle.id)
    ...
    const { data: recentLoad } = await supabase
      .from('sensor_data').select('value')
      .eq('vehicle_id', vehicle.id).eq('sensor_type', 'engineLoad')
  ...
  return ok('Fuel predictions generated', corsHeaders);
}

async function handleInsightGeneration(supabase: any, corsHeaders: Record<string, string>) {
  const { data: vehicles } = await supabase.from('vehicles').select('*');
  if (!vehicles) return ok('No vehicles', corsHeaders);
  ...
  for (const vehicle of vehicles) {
    ...
    const { data: idleReadings } = await supabase
      .from('sensor_data').select('value')
      .eq('vehicle_id', vehicle.id).eq('sensor_type', 'speed')
      .gte('timestamp', since).lt('value', 1);

    const idleMinutes = (idleReadings?.length ?? 0);
    const idleHours   = idleMinutes / 3600;
    const fuelPrice   = vehicle.fuel_price_per_litre ?? 100;
    const idleWasteLitres = idleHours * 0.8;
    const idleWasteCost   = idleWasteLitres * fuelPrice * 30;
  ...
  return ok('Insights generated', corsHeaders);
}

async function handleCostPrediction(supabase: any, corsHeaders: Record<string, string>) {
  const { data: vehicles } = await supabase.from('vehicles').select('*');
  if (!vehicles) return ok('No vehicles', corsHeaders);

  for (const vehicle of vehicles) {
    const { data: history } = await supabase
      .from('historical_cost_data').select('*')
      .eq('vehicle_id', vehicle.id)
      .order('date', { ascending: false }).limit(20);

    const { data: behavior } = await supabase
      .from('driver_behavior').select('driver_score, average_engine_load')
      .eq('vehicle_id', vehicle.id)
    ...
    const avgMonthly = history && history.length > 0
    ...
    await supabase.from('cost_predictions').insert({
      ...
      factors: {
        driving_impact:    wearFactor > 0
          ? `Aggressive driving adds ${(wearFactor * 100).toFixed(0)}% to maintenance`
          : 'Optimized driving reducing costs',
        historical_trend: history?.length
          ? 'Based on last 3 months spending'
          : 'Baseline estimate',
      },
    });
  }

  return ok('Cost predictions generated', corsHeaders);
}

async function handleRouteOptimization(supabase: any, url: URL, corsHeaders: Record<string, string>) {
  ...
  for (const dest of destinations || []) {
    const distance = Math.sqrt(
      Math.pow(dest.latitude - originLat, 2) + Math.pow(dest.longitude - originLng, 2)
    ) * 111;
    ...
    results.push({
      ...
      traffic_score:    Math.min(10, trafficScore).toFixed(1),
      efficiency_score: efficiencyScore.toFixed(1),
      recommendation:   efficiencyScore > 7 ? 'HIGHLY RECOMMENDED' : efficiencyScore > 4 ? 'VIABLE' : 'AVOID',
    });
  }
  ...
  if (best) {
    await supabase.from('optimized_routes').insert({
      ...
      destination_id:       best.dest_id,
      estimated_distance_km: best.distance,
      traffic_score:        best.traffic_score,
      fuel_efficiency_score: best.efficiency_score,
    });
  }

  return new Response(JSON.stringify({ success: true, best_route: best, all_options: results }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}

function ok(message: string, corsHeaders: Record<string, string>) {
...
async function handleAnomalyDetection(admin: any) {
  ...
  const { data: vehicles } = await admin
    .from('vehicles').select('id, name, vin, fleet_id').eq('is_active', true);

  for (const vehicle of (vehicles ?? [])) {
    const { data: logs } = await admin
      .from('vehicle_logs').select('latitude, longitude, speed, ignition_status, timestamp, is_mock_gps')
      .eq('vehicle_id', vehicle.id).gte('timestamp', since)
    ...
    for (let i = 1; i < logs.length; i++) {
      ...
      if (!curr.ignition_status && !prev.ignition_status && dist > 0.2) {
        const { data: existing } = await admin
          .from('vehicle_events').select('id')
          .eq('vehicle_id', vehicle.id).eq('event_type', 'unauthorized_movement')
        ...
        if (!existing) {
          ...
          const { data: fleet } = await admin.from('fleets')
            .select('whatsapp_number').eq('id', vehicle.fleet_id).single();
...
async function handleGapDetection(admin: any) {
  ...
  const { data: activeTrips } = await admin
    .from('trips').select('id, vehicle_id, start_time, fleet_id:vehicles(fleet_id)')
    .eq('status', 'active');

  for (const trip of (activeTrips ?? [])) {
    const { data: logs } = await admin
      .from('vehicle_logs').select('timestamp')
      .eq('vehicle_id', trip.vehicle_id).gte('timestamp', trip.start_time)
      .order('timestamp', { ascending: true });
    ...
    for (let i = 1; i < logs.length; i++) {
      ...
      if (gapMin > 5) {
        ...
        const { data: existing } = await admin
          .from('vehicle_events').select('id')
          .eq('vehicle_id', trip.vehicle_id).eq('event_type', 'trip_gap')
          .gte('created_at', logs[i - 1].timestamp).lte('created_at', logs[i].timestamp)
          .maybeSingle();
        ...
        if (!existing) {
          ...
          await admin.from('vehicle_events').insert({
            ...
            metadata:    { trip_id: trip.id, gap_start: logs[i - 1].timestamp, gap_end: logs[i].timestamp, gap_minutes: gapMin },
          });
...
async function handleTripScoring(admin: any) {
  ...
  const { data: trips } = await admin
    .from('trips').select('id, vehicle_id, start_time, end_time, gap_count, gap_duration_minutes')
    .eq('status', 'completed').is('data_confidence_score', null).gte('end_time', since);
  ...
  for (const trip of (trips ?? [])) {
    ...
    const { count: anomalyCount } = await admin
      .from('vehicle_events').select('*', { count: 'exact', head: true })
